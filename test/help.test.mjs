...
test('formatCliHelpText explains new upgrade, port, and shell commands', () => {
  const text = formatCliHelpText();
  assert.match(text, /vpnctl dev init/);
  assert.match(text, /vpnctl upgrade/);
  assert.match(text, /vpnctl config set-ports/);
  assert.match(text, /vpnctl shell install/);
  assert.match(text, /分步进度/);
  assert.match(text, /新会话运行 codex/);
});
